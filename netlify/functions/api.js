...
<head>
    ...
    <link rel="shortcut icon" href="https://cresce-ao.netlify.app/logo.png" type="image/x-icon" />
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge { display: inline-flex; align-items: center; gap: 8px; background: #DCFCE7; color: var(--success); border: 1px solid #BBF7D0; border-radius: 999px; padding: 8px 20px; font-size: 13px; font-weight: 600; letter-spacing: .04em; }
        .badge svg { width: 16px; height: 16px; stroke-dasharray: 50; stroke-dashoffset: 50; animation: dash 0.6s ease 0.4s forwards; }
        ...
        .card-header { background: var(--dark); padding: 32px 40px; display: flex; align-items: center; justify-content: space-between; position: relative; overflow: hidden; }
        .card-header::before { content: ''; position: absolute; right: -40px; top: -40px; width: 200px; height: 200px; border-radius: 50%; background: radial-gradient(circle, rgba(241, 90, 43, .25) 0%, transparent 70%); }
        .logo-mark { display: flex; align-items: center; gap: 10px; }
        .logo-name { color: #fff; font-size: 18px; font-weight: 700; letter-spacing: -.01em; }
        .invoice-num { font-family: 'DM Mono', monospace; color: rgba(255, 255, 255, .5); font-size: 12px; letter-spacing: .06em; text-align: right; }
        ...
        .meta-item .value.highlight { color: var(--orange); }
        ...
        .event-name { font-size: 17px; font-weight: 700; color: var(--ink); line-height: 1.3; }
        ...
        .status-pill { display: inline-flex; align-items: center; gap: 6px; background: #DCFCE7; color: var(--success); border-radius: 999px; padding: 5px 14px; font-size: 12px; font-weight: 700; letter-spacing: .04em; }
        .status-dot { width: 7px; height: 7px; background: var(--success); border-radius: 50%; }
        .card-footer { background: var(--bg); border-top: 1px solid var(--line); padding: 24px 40px; display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap; }
        ...
        .qr-code-label { font-size: 11px; color: var(--muted); font-family: 'DM Mono', monospace; letter-spacing: .05em; }
...
<body>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="meta-grid">
                ...
                <div class="meta-item"><div class="label">Email</div><div class="value" id="userEmail" style="font-size:13px;">—</div></div>
            ...
            <div class="event-box"><div class="event-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg></div><div><div class="event-name" id="eventName">—</div><div class="event-sub" id="eventId">ID do Evento: —</div></div></div>
            <table class="items-table"><thead><tr><th>Descrição</th><th>Qtd.</th><th>Unit.</th><th>Total</th></tr></thead><tbody id="lineItems"><tr><td><div class="item-name" id="ticketType">—</div><div class="item-desc">Ingresso para o evento</div></td><td id="qty">—</td><td id="unitPrice">—</td><td id="lineTotal">—</td></tr></tbody></table>
            ...
            <div class="qr-section" id="qrSection" style="display:none;"><div class="label" style="margin-bottom:12px;">Código do Ingresso</div><div class="qr-box"><canvas id="qrCanvas" width="120" height="120"></canvas></div><div class="qr-code-label" id="ticketCode">—</div></div>
        </div>
        <div class="card-footer"><div class="footer-note"><strong>Cresce.AO</strong><br>Este documento serve como comprovativo oficial de compra.<br>Guarda este ingresso para apresentar no evento.</div><div class="footer-note" style="text-align:right;"><strong id="sessionId" style="font-family:'DM Mono',monospace;font-size:11px;">—</strong><br>Powered by Stripe</div></div>
    </div>
    ...
    <script>
        ...
        function fmtDate(ts) { const d = ts ? new Date(ts * 1000) : new Date(); return d.toLocaleDateString('pt-AO', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }); }
